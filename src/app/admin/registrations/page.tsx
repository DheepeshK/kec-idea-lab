...
import { ClipboardList, Mail, Phone, Calendar, User, ExternalLink, Trash2 } from 'lucide-react';
...
export default function AdminRegistrationsPage() {
  ...
  const handleDelete = async (id: string, name: string) => {
    if (!window.confirm(`Delete registration from "${name}"?`)) return;
    try {
    ...
    <div className="space-y-8">
      ...
      {loading ? (
        <div className="text-center py-20">
          <LoadingSpinner />
          <p className="text-xs text-text-secondary mt-2">Loading registrations...</p>
        </div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                    <div className="space-y-2 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <Badge variant="primary" id={`reg-badge-${reg._id}`}>
                          {reg.rollNoDept}
                        </Badge>
                      </div>
